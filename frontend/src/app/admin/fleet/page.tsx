...
export default function AdminFleetPage() {
  ...
  return (
    <div className="space-y-6 p-6 text-white lg:p-8">
      ...
      {/* Version spread */}
      <section className="rounded-2xl border border-white/8 bg-[#0a0f1e] p-5">
        <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
          <Rocket className="size-4 text-violet-400" />
          Version spread
          {latest && <span className="ml-2 text-xs font-normal text-white/40">latest released: <span className="text-emerald-400">{latest}</span></span>}
          {data && data.updating > 0 && <span className="ml-auto text-xs font-normal text-amber-300">{data.updating} updating…</span>}
        </div>
        {versions.length === 0 ? (
          <p className="text-sm text-white/40">No paired TVs yet.</p>
        ) : (
            ...
            {versions.map(([version, count]) => {
              ...
              return (
                <div key={version} className="flex items-center gap-3">
                  <span className={`w-28 shrink-0 truncate font-mono text-xs ${isLatest ? 'text-emerald-400' : 'text-white/70'}`}>
                    {version}{isLatest && ' ✓'}
                  </span>
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/5">
                    <div className={`h-full rounded-full ${isLatest ? 'bg-emerald-500' : 'bg-violet-500'}`} style={{ width: `${pct}%` }} />
                  </div>
                  <span className="w-10 shrink-0 text-right text-xs text-white/50">{count}</span>
                </div>
              )
            })}
          </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-white/8 bg-[#0a0f1e]">
        ...
        {isLoading ? (
          ...
          <p className="p-10 text-center text-sm text-white/40">No TVs paired across the fleet yet.</p>
        ) : (
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {data.screens.map((s) => (
                  <tr key={s.id} className="transition-colors hover:bg-white/[0.02]">
                    ...
                    <td className="p-4 text-xs text-white/60">{s.organization_name}</td>
                    <td className="p-4">
                      <span className={`font-mono text-xs ${latest && s.app_version === latest ? 'text-emerald-400' : 'text-white/80'}`}>
                        {s.app_version || '—'}
                      </span>
                    </td>
                    <td className="p-4 text-xs text-white/50">{s.target_version_code ?? '—'}</td>
                    <td className="p-4"><UpdateChip screen={s} /></td>
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 text-xs ${s.online ? 'text-emerald-400' : 'text-white/40'}`}>
                        <span className={`size-1.5 rounded-full ${s.online ? 'bg-emerald-400' : 'bg-white/30'}`} />
                        {s.online ? 'Online' : 'Offline'}
                      </span>
                    </td>
                    <td className="p-4 pr-5 text-xs text-white/40">{s.last_seen ? new Date(s.last_seen).toLocaleString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
