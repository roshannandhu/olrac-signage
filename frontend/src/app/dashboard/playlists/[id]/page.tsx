...
import { ArrowLeft, CalendarClock, Check, ChevronDown, Clock3, GripVertical, ListVideo, Plus, Search, Sparkles, Trash2 } from 'lucide-react'
...
import { dateTimeLocal, loopDuration } from '@/lib/format'
    ...
    <Card className="mb-5 border-0 bg-card py-0 shadow-[0_1px_2px_rgba(15,23,42,.04)] ring-1 ring-hairline">
      <CardContent className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-center">
        <div>
          ...
          <fieldset disabled={disabled || pending} className="mt-5 grid gap-4 sm:grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)]">
      ...
      <CardContent className="p-4 sm:p-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-muted-foreground/70 mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
              <span className="inline-flex items-center gap-1"><Clock3 className="size-3" /> {item.duration}s</span>
        ...
        {open && (
          <div id={`item-settings-${item.id}`} className="border-hairline mt-5 border-t pt-5">
            <fieldset disabled={!canEdit || saving} className="grid gap-5 lg:grid-cols-2">
              <div>
...
export default function PlaylistBuilderPage() {
  ...
  if (playlistQuery.isLoading) return <div className="space-y-6"><Skeleton className="h-24" /><div className="grid gap-6 lg:grid-cols-[1fr_340px]"><Skeleton className="h-[520px]" /><Skeleton className="h-[520px]" /></div></div>
  ...
  return (
    <div className="space-y-8">
      <Link href="/dashboard/playlists" className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back to playlists</Link>
      <PageHeader eyebrow="Playlist builder" title={playlist.name} description="Set the loop order, control each handoff, and schedule exactly when every item may play." actions={<div className="flex items-center gap-2"><Badge variant="outline">{items.length} items</Badge><Badge variant="outline">{loopDuration(totalDuration)} loop</Badge>{!canEdit && <Badge variant="warning">View only</Badge>}</div>} />
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="rounded-2xl bg-card p-4 shadow-[0_1px_2px_rgba(15,23,42,.04)] ring-1 ring-hairline xl:sticky xl:top-6" aria-labelledby="library-title">
          ...
          <div className="mt-4 max-h-[660px] space-y-2 overflow-y-auto pr-1">
            {contentQuery.isLoading ? Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-20" />) : availableContent.map((content) => <div key={content.id} className="flex items-center gap-3 rounded-xl border border-hairline p-2.5 hover:bg-muted"><MediaThumbnail item={content} className="size-12 shrink-0 rounded-lg" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-foreground">{content.name}</p><p className="mt-0.5 text-xs capitalize text-muted-foreground/70">{content.type}</p></div><Button size="icon-sm" variant="outline" disabled={!canEdit || addMutation.isPending} onClick={() => addMutation.mutate(content.id)} aria-label={`Add ${content.name}`}><Plus /></Button></div>)}
