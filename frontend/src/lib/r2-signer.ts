...
/**
 * Synchronously converts any `s3://`, `r2://`, or `/api/media/<key>` storage location into a 7-day
 * presigned Cloudflare R2 URL.
 *
 * Runs with zero dependencies on external network or server uptime, serving thumbnails directly
 * from Cloudflare R2 CDN edge.
 */
export function presignR2Url(keyOrUrl: string | null | undefined): string | undefined {
  ...
  if (keyOrUrl.startsWith('s3://') || keyOrUrl.startsWith('r2://')) {
    cleanKey = keyOrUrl.replace(/^(s3|r2):\/\//, '').replace(/^\/+/, '')
  } else if (keyOrUrl.includes('/api/media/')) {
    cleanKey = keyOrUrl.split('/api/media/')[1]?.split('?')[0]?.replace(/^\/+/, '')
  } else if (keyOrUrl.startsWith('org-')) {
    cleanKey = keyOrUrl.replace(/^\/+/, '')
  }
